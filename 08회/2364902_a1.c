...
void input_grade(struct student d[], int n);
...
void printout(struct student d[], int n, char file[]){
    ...
    for(int i = 0; i < n; i++){
        fprintf(fp, "%4d %-20s %3d        %c\n", d[i].num, d[i].name, d[i].score, d[i].grade);
    }
    fclose(fp);
}
...
void input_grade(struct student d[], int n){
    for(int i = 0; i < n; i++){
        int score = d[i].score / 10;
        switch(score){
            case 10:
            case 9://when score is 90 ~ 100
                d[i].grade = 'S';
                break;
            case 8://when score is 80 ~ 89
                d[i].grade = 'A';
                break;
            case 7://when score is 70 ~ 79
                d[i].grade = 'B';
                break;
            case 6://when score is 60 ~ 69
                d[i].grade = 'C';
                break;
            default://when score 59 ~ 0
                d[i].grade = 'D';
        }
    }
}
